...
#define RYGPhotoW 54
#define RYGPhotoH 54
#define RYGPhotoMargin 5

@implementation RYGPhotosView
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // 初始化9个子控件
        for (int i = 0; i<9; i++) {
            RYGPhotoView *photoView = [[RYGPhotoView alloc] init];
...
- (void)photoTap:(UITapGestureRecognizer *)recognizer
{
    ...
    for (int i = 0; i<count; i++) {
        ...
        NSString *iwphoto = self.photos[i];
//        NSString *photoUrl = [iwphoto stringByReplacingOccurrencesOfString:@"thumbnail" withString:@"bmiddle"];
        mjphoto.url = [NSURL URLWithString:iwphoto]; // 图片路径
...
- (void)setPhotos:(NSArray *)photos
{
    ...
    for (int i = 0; i<self.subviews.count; i++) {
        // 取出i位置对应的imageView
        RYGPhotoView *photoView = self.subviews[i];
        ...
        if (i < photos.count) {
            ...
            // 传递模型数据
            photoView.imgUrl = photos[i];
            
            // 设置子控件的frame
            int maxColumns = (photos.count == 4) ? 2 : 3;
            int col = i % maxColumns;
            int row = i / maxColumns;
            CGFloat photoX = col * (RYGPhotoW + RYGPhotoMargin);
            CGFloat photoY = row * (RYGPhotoH + RYGPhotoMargin);
            photoView.frame = CGRectMake(photoX, photoY, RYGPhotoW, RYGPhotoH);
...
+ (CGSize)photosViewSizeWithPhotosCount:(int)count
{
    ...
    // 高度
    CGFloat photosH = rows * RYGPhotoH + (rows - 1) * RYGPhotoMargin;
    
    // 总列数
    int cols = (count >= maxColumns) ? maxColumns : count;
    // 宽度
    CGFloat photosW = cols * RYGPhotoW + (cols - 1) * RYGPhotoMargin;
