...
// 底部圈的长度
@property (nonatomic) NSInteger inLineWidth;
...
{
    return [self initWithFrame:frame lineWidth:0 inLineWidth:0 atValue:0 totalValue:0 color:nil];
}

//  Designated Initializer
- (instancetype)initWithFrame:(CGRect)frame lineWidth:(NSInteger)width inLineWidth:(NSInteger)inLineWidth
...
- (void)setLineWidth:(NSInteger)width  inLineWidth:(NSInteger)inLineWidth atValue:(NSInteger)value
...
- (void)drawBackgroundCircle
{
    ...
    bottomLayer.lineWidth = self.inLineWidth;
...
{
    CGSize desSize = RYG_TEXTSIZE(self.description,[UIFont systemFontOfSize:self.descriptionFontSize]);
    NSString  *format = @"%d%%";
    NSString *rate = [NSString stringWithFormat:format,self.value];
    CGSize size = RYG_TEXTSIZE(rate,[UIFont systemFontOfSize:self.descriptionFontSize]);
    CGFloat height = desSize.height + size.height;
    
    _labelDes = [[UILabel alloc]initWithFrame:CGRectMake(0, (self.height - height - 5) / 2, self.width, desSize.height)];
    _labelDes.font = [UIFont systemFontOfSize:self.descriptionFontSize];
    _labelDes.textColor = ColorName;
    _labelDes.textAlignment = NSTextAlignmentCenter;
    _labelDes.text = self.descriptionString;
    [self addSubview:_labelDes];
    self.descriptionLabel = _labelDes;
    
    _labelNum = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_labelDes.frame) + 5, self.width, size.height)];
    _labelNum.font = [UIFont systemFontOfSize:self.valuesFontSize];
    _labelNum.textAlignment = NSTextAlignmentCenter;
    _labelNum.text = rate;
    _labelNum.textColor = ColorRateTitle;
    [self addSubview:_labelNum];
    self.label = _labelNum;

}

- (void)setLabelDesColor:(UIColor *)color {
    _labelDes.textColor = color;
    _descriptionLabel.textColor = color;
}

- (void)setlabelNumColor:(UIColor *)color {
