//
//  RYGFeedPackageViewController.m
//  shoumila
//
...
#import "RYGFeedPackageViewController.h"
...
@interface RYGFeedPackageViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation RYGFeedPackageViewController
@synthesize mPackageTableView;

-(void)viewWillAppear:(BOOL)animated
{
    [super viewWillAppear:animated];
    
    [MobClick beginLogPageView:@"我的套餐"];
}
...
- (void)initSubViews
{
    mPackageTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT-64) style:UITableViewStylePlain];
    mPackageTableView.delegate = self;
    mPackageTableView.dataSource = self;
    [mPackageTableView registerClass:[UITableViewCell class] forCellReuseIdentifier:@"Cell"];
    mPackageTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    __weak __typeof(&*self)weakself = self;
    [mPackageTableView addLegendHeaderWithRefreshingBlock:^{
        [weakself aRefresh];
    }];
    __weak __typeof(&*self)weakself1 = self;
    [mPackageTableView addLegendFooterWithRefreshingBlock:^{
        [weakself1 loadData];
    } noMoreDataTitle:@"没有更多数据"];
    [self.view addSubview:mPackageTableView];
...
- (void)loadData{
    ...
    param.count = @"5";
    param.next = _next;
    [RYGHttpRequest postWithURL:Feed_myPackage params:[param keyValues] success:^(id json) {
        ...
        [self.mPackageTableView.header setState:MJRefreshHeaderStateIdle];
...
- (void)reloadRankingTable {
    [self.mPackageTableView.footer endRefreshing];
    if (!_isMoreData) {
        [self.mPackageTableView.footer setState:MJRefreshFooterStateNoMoreData];
    }
