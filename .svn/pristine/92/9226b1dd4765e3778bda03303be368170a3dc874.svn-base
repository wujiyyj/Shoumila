//
//  RYGWeekRankingViewController.m
//  周榜单
//
...
@interface RYGWeekRankingViewController ()<UITableViewDataSource,UITableViewDelegate,RYGWeekRankTableViewCellDelegate>
...
@property(nonatomic,strong)RYGWeekRankingModel *rankingModel;
// 下一页的标识
...
@implementation RYGWeekRankingViewController
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    [MobClick beginLogPageView:@"周排行榜"];
}
...
- (void)aRefresh {
    _rankingList = [[NSMutableArray alloc]init];
    _next = 0;
    _isMoreData = YES;
    [self LoadData];
}
...
- (void)setupMyRankView {
    ...
    _lblAllProfitTitle.text = @"本周我的总利润：";
...
- (void)LoadData{
    ...
    [RYGHttpRequest postWithURL:Ranking_Week_list params:rankingListParam.keyValues success:^(id json) {
        NSMutableDictionary *dic = [json valueForKey:@"data"];
        self.rankingModel = [RYGWeekRankingModel objectWithKeyValues:dic];
...
{
    return WeekRankCellHeight;
}

-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    static NSString *weekRankCellIdentify=@"WeekRankCellIdentify";
    RYGWeekRankTableViewCell *cell=[tableView dequeueReusableCellWithIdentifier:weekRankCellIdentify];
    if (!cell) {
        cell=[[RYGWeekRankTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:weekRankCellIdentify];
    ...
    if ([RYGUtility isLogin]) {
        ...
        [RYGHttpRequest postWithURL:User_Attention params:attentionParam.keyValues success:^(id json) {
            ...
            if (code && code.integerValue == 0) {
                RYGWeekPersonModel *model = [weakself.rankingList objectAtIndex:index];
