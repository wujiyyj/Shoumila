...
#import "RYGWeekRankTableViewCell.h"
...
// 近10场推荐情况
@property(nonatomic,strong)RYGRecentRecomendView *recentRecomendView;
// 最近
@property(nonatomic,strong)UILabel *lblRecentTitle;
@property(nonatomic,strong)RYGRecentView *recentView;
...
@implementation RYGWeekRankTableViewCell
...
// 初期化cell
- (void)setupCell {
    self.contentView.frame = CGRectMake(self.contentView.origin.x, self.contentView.origin.y, SCREEN_WIDTH, WeekRankCellHeight);
     self.selectionStyle = UITableViewCellSelectionStyleNone;
    
    // 奖牌排名
    _weekRankMedalImageView = [[UIImageView alloc]initWithFrame:CGRectMake(WeekRankTableViewCell_Left_Magin, WeekRankTableViewCell_Top_Magin, 21, 30)];
    [self.contentView addSubview:_weekRankMedalImageView];
    
    // 奖牌排名
    _lblWeekRankMedal = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMinX(_weekRankMedalImageView.frame), WeekRankTableViewCell_Top_Magin, 21, 30)];
    ...
    // 头像
    _userLogoImageView = [[UIImageView alloc]initWithFrame:CGRectMake(WeekRankTableViewCell_Logo_Left_Magin, WeekRankTableViewCell_Top_Magin, 32, 32)];
    ...
    // 新帖字数
    // 添加一个提醒数字按钮
    UIControl *control = [[UIControl alloc]initWithFrame:CGRectMake(WeekRankTableViewCell_Logo_Left_Magin, WeekRankTableViewCell_Top_Magin, 32, 32)];
    ...
    _otherPersonCtl = [[UIControl alloc]initWithFrame:CGRectMake(CGRectGetMinX(_weekRankMedalImageView.frame), 0, self.width - CGRectGetMinX(_lblFans.frame), WeekRankCellHeight - 15 - 60)];
    ...
    // 关注
    _attentionCtrl = [[UIControl alloc]initWithFrame:CGRectMake(self.contentView.width - WeekRankTableViewCell_Right_Magin - WeekRankTableViewCell_Attention_Width, WeekRankTableViewCell_Attention_Top_Magin, WeekRankTableViewCell_Attention_Width, 23)];
    ...
    _attentionImageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, WeekRankTableViewCell_Attention_Width, 23)];
    _attentionImageView.image  = [UIImage imageNamed:@"un_attention"];
    [_attentionCtrl addSubview:_attentionImageView];

    // 胜率
    _winRateCircleChart = [[AFCircleChart alloc] initWithFrame:CGRectMake(20,WeekRankCellHeight - 15 - 60, 60, 60)];
    ...
    _lblAverageLevelTitle = [[UILabel alloc]initWithFrame:CGRectMake(self.contentView.width - WeekRankTableViewCell_Right_Magin - averageLevelTitleSize.width, CGRectGetMaxY(_attentionCtrl.frame) + WeekRankTableViewCell_Recomend_Top_Magin, averageLevelTitleSize.width, averageLevelTitleSize.height)];
    ...
    // 推荐胜／周推荐
    CGSize recommendWinTitleSize = RYG_TEXTSIZE(@"推荐胜/周推荐", [UIFont systemFontOfSize:9]);
    _lblRecommendWinTitle = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMinX(_lblAverageLevelTitle.frame) - 14 - recommendWinTitleSize.width,CGRectGetMaxY(_attentionCtrl.frame) + WeekRankTableViewCell_Recomend_Top_Magin, recommendWinTitleSize.width, recommendWinTitleSize.height)];
    _lblRecommendWinTitle.font = [UIFont systemFontOfSize:9];
    _lblRecommendWinTitle.textColor = ColorSecondTitle;
    _lblRecommendWinTitle.textAlignment = NSTextAlignmentLeft;
    _lblRecommendWinTitle.text = @"推荐胜/周推荐";
    ...
    // 近10场推荐情况
    _recentRecomendView = [[RYGRecentRecomendView alloc]initWithFrame:CGRectMake(CGRectGetMinX(_lblRecommendWinTitle.frame), CGRectGetMaxY(_weekRecommendView.frame) + 6, self.contentView.width - CGRectGetMinX(_lblRecommendWinTitle.frame) - 10 , [RYGRecentView recentSize].height) ];
    [self.contentView addSubview:_recentRecomendView];
    
    // 最近
    _recentView = [[RYGRecentView alloc]initWithFrame:CGRectMake(CGRectGetMinX(_lblRecommendWinTitle.frame), CGRectGetMaxY(_recentRecomendView.frame) + 4, self.contentView.width - CGRectGetMinX(_lblRecommendWinTitle.frame) - 15 , [RYGRecentView recentSize].height)];
    [self.contentView addSubview:_recentView];
    
    UIView *lineView = [[UIView alloc]initWithFrame:CGRectMake(0, WeekRankCellHeight - SeparatorLineHeight, self.contentView.width, SeparatorLineHeight)];
...
// 根据数据重新设置cell单元格
- (void)setPersonRankingModel:(RYGWeekPersonModel *)personRankingModel {
...
- (void)updateCell {
    ...
    if ([self.personRankingModel.ranking isEqualToString:@"1"] || [self.personRankingModel.ranking isEqualToString:@"2"] || [self.personRankingModel.ranking isEqualToString:@"3"]) {
        _weekRankMedalImageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"week_rank_ medal%@",self.personRankingModel.ranking]];
        ...
        _lblWeekRankMedal.frame = CGRectMake(CGRectGetMinX(_weekRankMedalImageView.frame), WeekRankTableViewCell_Top_Magin, width, 30);
    }
    ...
    CGFloat userNameWidth = MIN(userNameSize.width, self.contentView.width - WeekRankTableViewCell_Right_Magin -  WeekRankTableViewCell_Attention_Width - CGRectGetMaxX(_userLogoImageView.frame) - WeekRankTableViewCell_Name_Left_Magin);
    _lblUserName.frame = CGRectMake(CGRectGetMaxX(_userLogoImageView.frame) + WeekRankTableViewCell_Name_Left_Magin, WeekRankTableViewCell_Top_Magin,userNameWidth, userNameSize.height);
    // 等级标志
    _grandMarkView.frame = CGRectMake(CGRectGetMaxX(_lblUserName.frame)+5, CGRectGetMinY(_lblUserName.frame), self.contentView.width - CGRectGetMaxX(_lblUserName.frame) - WeekRankTableViewCell_Attention_Width - 15 - 3, 15);
...
//    _grandMarkView.integralRank = self.personRankingModel.grade;
    ...
    [_weekRecommendView setRecommendWinGames:strWin recommendGames:strRecommend];
    // 平均推荐水位
    ...
    // 近10场推荐情况
    _recentRecomendView.recents = self.personRankingModel.recent;
    
    _recentView.recents = self.personRankingModel.recent;
