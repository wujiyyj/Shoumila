...
@interface RYGDynamicDetailViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,UITextFieldDelegate,facialViewDelegate,UITextViewDelegate,UIAlertViewDelegate>
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [MobClick beginLogPageView:@"动态详情"];
}
...
- (void)loadData{
    ...
    [RYGHttpRequest getWithURL:url params:[param keyValues] success:^(id json) {
        ...
        [commentList enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            RYGCommentModel *commentMode = obj;
            commentFrame.isNeedFooter = commentMode.thread.count>=3;
            commentFrame.commentModel = commentMode;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([RYGUtility getUserInfo].userid == commentModel.uid) {
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex==1) {
        ...
        param.del = @"1";
        param.comment_id = commentModel.comment_id;
